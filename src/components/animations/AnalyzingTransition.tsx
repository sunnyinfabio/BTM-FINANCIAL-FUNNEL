...
export function AnalyzingTransition({ onComplete }: AnalyzingTransitionProps) {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setActiveStep(2), 850);
  ...
  return (
    <div className="flex min-h-[520px] w-full flex-col items-center justify-center py-12 px-4 text-center select-none">
      {/* Top Header */}
      <motion.div
      ...
      >
        ...
        <h2 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-[#062039] mt-2">
          Mapping your requirements
        </h2>
        <p className="text-xs sm:text-sm text-slate-500 max-w-sm mx-auto font-normal">
          Correlating operational telemetry, institutional models, and capability architectures.
        </p>
      </motion.div>

      {/* Visual Diagnostic Flow Diagram: DATA -> BUSINESS -> PRIORITY -> BTM CAPABILITIES */}
      <div className="mt-8 w-full max-w-xs sm:max-w-sm flex flex-col items-center">
        {/* Node 1: DATA */}
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          className={`w-full flex items-center justify-between p-3 rounded-[6px] border transition-all duration-300 ${
            activeStep >= 0
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              <Database className="h-3.5 w-3.5 text-[#009345]" />
            </div>
        ...
        {/* Line 1 */}
        <div className="h-5 w-[2px] relative overflow-hidden bg-slate-200 my-0.5">
          <motion.div
        ...
        >
          <div className="flex items-center gap-3">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-[3px] ${
                activeStep >= 2 ? 'bg-[#062039] text-white' : 'bg-slate-200 text-slate-500'
              }`}
            >
          ...
          {activeStep > 2 && <CheckCircle2 className="h-4 w-4 text-[#009345]" />}
        </motion.div>
        ...
        <motion.div
          ...
          className={`w-full flex items-center justify-between p-3 rounded-[6px] border transition-all duration-300 ${
            activeStep >= 3
              ? 'border-[#009345] bg-emerald-50/60 ring-2 ring-[#009345] shadow-sm'
              : 'border-slate-200 bg-slate-50 text-slate-400'
          }`}
        >
          <div className="flex items-center gap-3">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-[3px] ${
                activeStep >= 3 ? 'bg-[#009345] text-white' : 'bg-slate-200 text-slate-500'
              }`}
            >
      ...
      {/* 4 Loading Dots Indicator */}
      <div className="mt-8 flex items-center gap-2">
        <span className="h-2 w-2 rounded-full bg-[#009345] animate-bounce [animation-delay:-0.45s]" />
